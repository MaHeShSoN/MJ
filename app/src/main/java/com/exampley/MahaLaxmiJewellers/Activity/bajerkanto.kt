...
class bajerkanto : AppCompatActivity(), ImageViewer, ButtonClicer {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_bajerkanto)

        try {
            supportActionBar!!.setBackgroundDrawable(ColorDrawable(resources.getColor(R.color.gold)))

            mSRef = FirebaseStorage.getInstance().getReference("bajerkanto")
            mDbRef = FirebaseDatabase.getInstance().getReference("uploads")
            supportActionBar!!.title = "बजरकंटो"


            db = FirebaseFirestore.getInstance()
            imgCollections = db.collection("bajerkanto")
...
//    }
    private fun setUpRecyclerView() {
        ...
        try {
            mRecyclerView = findViewById(R.id.recycler_view_bajerkanto)
        ...
        GlobalScope.launch(Dispatchers.IO) {
            ...
            docRef.get().await().reference.addSnapshotListener { value, error ->
                ...
                shareButton.setOnClickListener {
                    val builder: AlertDialog.Builder = AlertDialog.Builder(this@bajerkanto)
                    val layoutInflaterAndroid = LayoutInflater.from(this@bajerkanto)
                    ...
                    view.findViewById<TextView>(R.id.send_papa).setOnClickListener {
                        ...
                        val uri = getImageUri(this@bajerkanto, image)
